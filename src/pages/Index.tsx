...
const Index = () => {
  ...
  return (
    <>
      {/* SEO Schema Markup */}
      <SpeakableSchema
        headline="Champions Sports Bar & Grill - Hillsboro's Flavor & Game Hub"
        description={`Hillsboro's premier sports bar featuring NFL Sunday Ticket on 14 big screens, 20 craft beer taps, daily happy hour ${BUSINESS_DATA.happyHour.startDisplay}-${BUSINESS_DATA.happyHour.endDisplay}. Official Portland Timbers Pub Partner.`}
        cssSelectors={[".speakable-content", ".hero-content"]}
      />
        ...
        <section className="relative min-h-[100dvh] pt-20 md:pt-24 w-full flex items-center justify-center text-center text-white">
          ...
          <div className="relative z-20 container px-4 py-8 flex flex-col justify-center hero-content">
            ...
            <p className="mt-6 text-white text-xl md:text-2xl text-center drop-shadow-md">
              <a href="tel:5037476063" className="hover:underline">
                (503) 747-6063
              </a>
            </p>
              ...
              {isContentLoading ? (
                ...
                homepageContent?.map((content) =>
                  content.section_name === "qr_codes" ? (
                    <div
                      key={content.id}
                      className="text-center p-6 border border-border rounded-lg bg-card shadow-sm flex flex-col"
                    >
