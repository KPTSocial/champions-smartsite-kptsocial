...
export function ReferralWidget() {
  ...
  useEffect(() => {
    ...
    async function fetchProfileAndRefs() {
      ...
      let {
        data: profile
      } = await supabase.from("user_profiles").select("*").eq("id", uid).single();
      if (!profile) {
        // Generate a code from username/email prefix (or fallback to ID-6)
        const code = (session.user.user_metadata?.username?.toLowerCase() ?? session.user.email?.split("@")[0]?.toLowerCase() ?? uid.substring(0, 6)).replace(/[^a-z0-9]/gi, "");
        const {
          data: createdProfile
      ...
      let {
        data: referrals,
        error: refErr
      } = await supabase.from("referrals").select("*").eq("referrer_id", uid).order("created_at", {
        ascending: false
      });
      if (!referrals) referrals = [];

      // Calculate stats
      const joinedList = referrals.filter((r: Referral) => r.joined).map((r: Referral) => ({
        email: r.referred_email,
        date: new Date(r.created_at).toLocaleDateString()
      }));
      const totalPoints = referrals.reduce((sum: number, r: Referral) => sum + (r.points_awarded || 0), 0);
      if (isMounted) {
        setStats({
  ...
  if (!userId) {
    return <Card className="mb-6">
        ...
        <CardContent>
          <p className="text-muted-foreground mb-2">See your server for details</p>
        </CardContent>
      </Card>;
  }
  return <Card className="mb-6">
      ...
      <CardContent>
        <div className="mb-4 space-y-2">
          ...
          <div className="bg-accent p-2 rounded flex flex-col sm:flex-row sm:items-center gap-2">
            ...
            toast({
              title: "Copied!",
              description: "Your referral link is now on your clipboard."
            });
          }}>
        ...
        <div className="mt-4">
          ...
          {stats.joinedList.length > 0 ? <ul className="space-y-1">
              {stats.joinedList.map(({
            email,
            date
          }, i) => <li key={i} className="flex items-center gap-2 text-muted-foreground text-xs">
                  <CheckCircle className="w-3 h-3 text-green-500" /> <span>{email}</span> <span>({date})</span>
                </li>)}
            </ul> : <span className="text-xs text-muted-foreground">No friends have joined yet.</span>}
        </div>
      </CardContent>
    </Card>;
}